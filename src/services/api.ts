...
import { User, Skill, Activity, ProfileView } from '../types';
...
class ApiService {
  ...
  async getProfileViewsAnalytics(days: number = 30): Promise<any[]> {
  ...
  async getSkillsAnalytics(): Promise<any> {
    const response = await this.request('/analytics/skills');
